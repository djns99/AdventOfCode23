def part1(lines):
    grid = [[0 for _ in y] for y in lines]
    ...
    for y, l in enumerate(lines):
        ...
        for x, s in enumerate(l):
            if s in "0123456789":
                current *= 10
                current += int(s)
                has_valid = max(has_valid, grid[y][x])
            else:
                if has_valid:
                    sum += current
                current = 0
...
def part2(lines):
    grid = [[[] for _ in y] for y in lines]
    products = [[[] for _ in y] for y in lines]
    for y, l in enumerate(lines):
        for x, s in enumerate(l):
            if s == "*":
                for i in range(-1, 2):
