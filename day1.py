...
def find_match(s):
    regex = r"one|two|three|four|five|six|seven|eight|nine|zero|[0-9]"
    print(s)
    first = re.search(regex, s).group(0)
    last = None
    for i in range(len(s)):
        match = re.match(regex, s[-1-i:])
        if match:
            last = match.group(0)
            break
...
def part2(input_lines):
    return sum([find_match(l) for l in input_lines])
