...
for v in pipe_map.values():
    v.sort()


def valid_dir(c):
    return [(-x, -y) for x, y in pipe_map[c]]


def is_match(c, dir):
    return dir in pipe_map[c]
...
def build_path(grid):
    ...
    path = [None, pos]
    while True:
        for dx, dy in valid_dir(grid[pos[0]][pos[1]]):
...
def replace_start(grid, path):
    dir1 = sub(path[0], path[1])
    dir2 = sub(path[0], path[-1])
    dirs = list(sorted([dir1, dir2]))
    for k, v in pipe_map.items():
        if v == dirs:
            grid[path[0][0]][path[0][1]] = k[0]
            return grid
    return grid
...
def make_turning_number(new_dir, last_dir):
    if new_dir[0] == last_dir[1] and new_dir[1] == -last_dir[0]:
        return 1
    else:
        return -1
...
def part2(lines):
    ...
    path = build_path(grid)
    grid = replace_start(grid, path)

    path.append(path[0])
    turning_number = 0
    turns = []

    for i, (x, y) in enumerate(path[1:]):  # We are i+1
        c = grid[x][y]
        if is_corner(c):
            last_dir = sub(path[i + 1], path[i])
            dirs = pipe_map[c].copy()
            dirs.remove(last_dir)
            new_dir = dirs[0]
            turning_number += make_turning_number(new_dir, last_dir)


        turns.append(turning_number)
    ...
    while unvisited:
        ...
        while stack:
            ...
            for dx, dy in [(0, 1), (1, 0), (0, -1), (-1, 0)]:
                new_pos = top[0] + dx, top[1] + dy
                if new_pos in path_set:
                    i = path[1:].index(new_pos)  # Index of the item before
                    in_dir = sub(path[i + 1], path[i])
                    out_dir = sub(path[(i+2) % len(path[:-1])], path[i + 1])
                    ref_dir = (dx, dy)
                    def rot_vec(a):
                        return a[1], -a[0]
                    if ref_dir in [rot_vec(in_dir), rot_vec(out_dir)]:
                        side = 1
                    else:
                        side = -1
                    continue

                if new_pos not in unvisited:
                    continue
                count += 1
                unvisited.remove(new_pos)
                stack.append(new_pos)
                found.append(new_pos)

        print("Found", count, "on side", side)
        if side == turning_number/abs(turning_number):
            final_count += count
        for px, py in found:
            grid[px][py] = ' ' if side == 1 else '#'
